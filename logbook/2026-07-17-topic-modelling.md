...
- Data completeness finding: no salary files exist for 2020 or 2021 in the
  archive (confirmed via direct metadata search, not a pipeline artefact).
  Genuine gap in the published record.
...
- Debugged three real parsing bugs during development: (1) duplicate-named
  ...
  spreadsheet (january 2022 file has "Ndalese per mungesa te paperligjura"
  twice), causing column selection to return multiple columns instead of
  one. All three fixed and verified.
- Final dataset: 12,551 salary records, 364 unique MP names, 2018-2026
  (excluding 2020-2021 gap). 3 null values across gross_pay/net_pay fields.

  ### Update — Salary/MP record merge: match rate ceiling identified

- Merged salary dataset (12,551 records) against current MP records (236,
  representing Legjislatura e 10-të only) via normalized name matching.
- Match rate: 79.6% (9,993/12,551). Verified the ~20% unmatched are NOT a
  matching bug: spot-checked prominent unmatched names (e.g. Gramoz Ruçi,
  Vasilika Hysi) directly against MP records — zero matches found under
  any name variant, confirming these are genuinely former MPs from earlier
  legislatures not present in the current /anetaret endpoint snapshot.
...
- Root-caused duplicate salary records to re-uploaded source files (same
  month's data uploaded under multiple document IDs — e.g. April 2026
...
- Deduction outlier analysis reproduces and extends the OSINT article's
  finding (Bardhi, 41% deduction, Jan 2026): found Flamur Hoxha
  (Feb 2024, 71.4% deduction on full 310,250 ALL gross salary) as a
  comparable/more extreme case from independently parsed data.
...
entry: these AI mention counts were conflating AI with intelligence-services
usage of "inteligjencë". Genuine AI-specific count is 12, not 148.]
...
Correction 1 — topic clusters do exist, just outside the initial top-20 view.
Cross-checked which topics the 640 AI/cyber-keyword-matching chunks actually
fell into. Found genuine, coherent clusters further down the topic list:


Topic 76 (164 chunks): "kibernetike, kibernetik, sistemet, sulmet" — direct
cyberattack/cybersecurity incident discussion.
Topic 179 (62 chunks): "kibernetike, sulmet, hakerat, iranianë, sigurisë" —
explicitly references Iranian hackers, almost certainly discussion of the
2022 HomeLand Justice attack on AKSHI (CISA/FBI AA22-264A).
...
Correction 2 — AI vs. intelligence-services keyword conflation (important).
Topic 42's presence prompted a check of whether the original "inteligjenc*"
keyword count was conflating artificial intelligence with intelligence
services (SHISH, military/defense intelligence). Verified directly: of 177
total "inteligjenc*" occurrences across the corpus, only 12 are the specific
phrase "inteligjencë artificiale" (AI). The remaining 165 (93%) refer to
intelligence services in a defense/security sense, unrelated to AI. The
original keyword frequency analysis (246 "artificial_intelligence" hits)
substantially overcounted true AI references as a result.
...
(226 chunks across two BERTopic clusters, one explicitly referencing Iranian
hackers). This sharpens the dissertation's core contrast: parliament engaged
substantively with cybersecurity/infrastructure security following the 2022
attack, but essentially not with artificial intelligence as a governance
topic, prior to Diella's September 2025 appointment.

Methodological note. This sequence — an initial keyword-only finding,
refined by cross-checking against topic modelling, then further corrected by
a targeted phrase-level check — demonstrates the value of triangulating
...
- Ran LDA (gensim, 15 topics, custom Albanian stopword list, no_below=10/
  ...
  duking") are dominated by generic/procedural language rather than
...
- METHODOLOGICAL FINDING (directly serving proposal's stated comparison
  ...
  a real, coherent, substantively important discourse cluster (cybersecurity/
  AKSHI, ~226 of 138,164 chunks, 0.16% of corpus) that LDA's pure
  word-co-occurrence approach missed entirely, likely because the topic's
  raw frequency was too low relative to LDA's statistical requirements,
  despite being semantically coherent and substantively significant. This
  supports the methodological choice of BERTopic as primary, with LDA
  valuable specifically as a demonstration of embedding-based methods'
  advantage for identifying low-frequency but semantically distinct topics
  in heterogeneous parliamentary corpora.
